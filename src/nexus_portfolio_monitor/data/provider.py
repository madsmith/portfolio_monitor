...
class DataProvider:
    ...
    async def get_aggregate(self, symbol: AssetSymbol) -> Aggregate | None:
        ...
        try:
            ...
            for attempt in range(self._max_retries):
                try:
                    ...
                    if attempt < self._max_retries - 1:
                        logger.warning(f"Rate limit hit for {symbol}, waiting {self._rate_limit_sleep} seconds")
            ...
            if aggs and isinstance(aggs, list) and len(aggs) > 0:
                last_aggregate = None
                for agg in aggs:
                    if (agg.timestamp is not None and 
                        agg.open is not None and 
                        agg.high is not None and 
                        agg.low is not None and 
                        agg.close is not None and 
                        agg.volume is not None):
    ...
    def _find_missing_ranges(self, aggregates: List[Aggregate], from_: datetime, to: datetime) -> List[tuple[datetime, datetime]]:
        ...
        if start_time.microsecond > 0 or start_time.second > 0:
            ...
            start_time = (start_time + timedelta(minutes=1)).replace(second=0, microsecond=0)
        ...
        missing_timestamps: list[datetime] = sorted(expected_timestamps - existing_timestamps)
        ...
        for ts in missing_timestamps:
            if range_start is None:
                range_start = ts
            elif prev_ts and (ts - prev_ts).total_seconds() > 60:  # More than 1 minute gap
    ...
    async def _fetch_range(self, symbol: AssetSymbol, from_: datetime, to: datetime) -> List[Aggregate]:
        ...
        try:
            ...
            minutes_delta = int((to - from_).total_seconds() / 60) + 1  # +1 to include both endpoints
            ...
            logger.exception(f"Error fetching range for {symbol} ({from_} to {to}): {e}")

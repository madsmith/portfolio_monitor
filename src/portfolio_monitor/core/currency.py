"""
Currency module for Portfolio Monitor.
"""
...
def _to_decimal(value: Numeric) -> Decimal:
    """Convert a numeric value to Decimal, handling float precision."""
...
class Currency:
    ...
    @staticmethod
    def are_equivalent_currencies(
        currency_type1: CurrencyType, currency_type2: CurrencyType
    ) -> bool:
    ...
    def _parse_currency_type(self, currency_str: str) -> CurrencyType:
        ...
        raise ValueError(
            f"Unknown currency type: '{currency_str}'. Valid values are: {valid_currencies}"
        )
    ...
    def __add__(self, other: "Currency | Numeric") -> "Currency":
        """Add two Currency objects or a Currency and a number."""
        if isinstance(other, Currency):
            if not self.are_equivalent_currencies(
                self.currency_type, other.currency_type
            ):
                raise ValueError(
                    f"Cannot add currencies of different types: {self.currency_type.name} and {other.currency_type.name}"
                )
        ...
        return Currency(self._value + _to_decimal(other), self.currency_type)

    def __sub__(self, other: "Currency | Numeric") -> "Currency":
        ...
        if isinstance(other, Currency):
            if not self.are_equivalent_currencies(
                ...
                raise ValueError(
                    f"Cannot subtract currencies of different types: {self.currency_type.name} and {other.currency_type.name}"
                )
            # Keep the calling currency's type when doing math between equivalent currencies
            return Currency(self._value - other._value, self.currency_type)
    ...
    def __mul__(self, other: "Currency | Numeric") -> "Currency":
    ...
    def __truediv__(self, other: "Currency | Numeric") -> "Currency":
        """Division operation."""
        if isinstance(other, Currency):
    ...
    def __radd__(self, other: Numeric) -> "Currency":
        """Add a number to a Currency."""
        return Currency(_to_decimal(other) + self._value, self.currency_type)

    def __rsub__(self, other: Numeric) -> "Currency":
        """Subtract a Currency from a number."""
        return Currency(_to_decimal(other) - self._value, self.currency_type)

    def __rmul__(self, other: Numeric) -> "Currency":
    ...
    def __lt__(self, other: "Currency | Numeric") -> bool:
        if isinstance(other, Currency):
            if not self.are_equivalent_currencies(
                self.currency_type, other.currency_type
            ):
                raise ValueError(
                    f"Cannot compare currencies of different types: {self.currency_type.name} and {other.currency_type.name}"
                )
            return self._value < other._value
        return self._value < _to_decimal(other)

    def __le__(self, other: "Currency | Numeric") -> bool:
        if isinstance(other, Currency):
            if not self.are_equivalent_currencies(
                self.currency_type, other.currency_type
            ):
    ...
    def __gt__(self, other: "Currency | Numeric") -> bool:
        if isinstance(other, Currency):
            if not self.are_equivalent_currencies(
                self.currency_type, other.currency_type
            ):
    ...
    def __ge__(self, other: "Currency | Numeric") -> bool:
        if isinstance(other, Currency):
            if not self.are_equivalent_currencies(
                self.currency_type, other.currency_type
            ):
    ...
    @classmethod
    def parse_number(cls, value: "Numeric | str") -> "Currency":
        ...
        symbol_to_currency = {
            config.symbol: currency_type
            for currency_type, config in CURRENCY_CONFIGS.items()
        }

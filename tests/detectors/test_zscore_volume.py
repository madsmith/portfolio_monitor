...
def feed_alternating(ticker: AssetSymbol, detector: ZScoreVolumeDetector, n: int = 9) -> None:
    """Feed alternating 1100/900 volumes to build history with non-zero std dev (~100)."""
    for i in range(n):
        vol = 1100.0 if i % 2 == 0 else 900.0
        detector.update(make_agg(ticker, 100.0, i, volume=vol))
...
class TestZScoreVolumeDetector:
    ...
    def test_no_alert_zero_std_dev(self):
        ...
        for i in range(30):
            detector.update(make_agg(TICKER, 100.0, i, volume=1000.0))
    ...
    def test_no_alert_below_threshold(self):
        ...
        detector.update(make_agg(TICKER, 100.0, 9, volume=1050.0))
        assert detector.get_current_alert(TICKER) is None
    ...
    def test_fires_rising(self):
        ...
        detector.update(make_agg(TICKER, 100.0, 30, volume=5000.0))
    ...
    def test_no_alert_falling_volume(self):
        ...
        detector.update(make_agg(TICKER, 100.0, 9, volume=100.0))
        assert detector.get_current_alert(TICKER) is None

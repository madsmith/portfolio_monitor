...
def feed_baseline(symbol: AssetSymbol, detector: VolumeSpikeDetector, n: int, volume: float = 1000.0) -> None:
    for i in range(n):
        detector.update(make_agg(symbol, 100.0, i, volume=volume))
...
class TestVolumeSpikeDetector:
    ...
    def test_fires_rising(self):
        ...
        feed_baseline(TICKER, detector, 30)
        assert detector.is_primed(TICKER)
        # mean = (29×1000 + 5000) / 30 = 1166.67  →  5000 ≥ 1166.67×2 ≈ 2333.33
        detector.update(make_agg(TICKER, 100.0, 31, volume=5000.0))
    ...
    def test_no_alert_falling_volume(self):
        ...
        detector.update(make_agg(TICKER, 100.0, 31, volume=100.0))
        assert detector.get_current_alert(TICKER) is None
    ...
    def test_update_preserves_id(self):
        ...
        detector.update(make_agg(TICKER, 100.0, 32, volume=6000.0))
    ...
    def test_clears_when_condition_ends(self):
        ...
        detector.update(make_agg(TICKER, 100.0, 32, volume=1000.0))
        assert detector.get_current_alert(TICKER) is None
